...
MANUAL_POINT_2 = (400, 300)  # 第二个待发送的指定坐标
...
# 【新增】串口发送状态：1=待发第一个手动坐标，2=待发第二个手动坐标，3=自动路径规划
                ...
                if cov >= END_THRESHOLD:
                    ...
                    if pos is None:
                        out = 'NA\r\n'
                         ...
                         x_scaled = int(pos[0] / 20)  # X坐标除以20
                         y_scaled = int(pos[1] / 20)  # Y坐标除以20
                         out = f'X{x_scaled}Y{y_scaled}\r\n'  # 发送缩放后的坐标
                        #out = f'X{pos[0]}Y{pos[1]}\r\n'
                try:
            ...
            if line.lower() == 'ok':
              ...
              if cov >= END_THRESHOLD:
                  ...
                  if pos is None:
                      ...
                      # 2. 缩放并强制取整（避免小数）
                      x_scaled = int(round(x / 20))  # round()四舍五入，int()转整数
                      y_scaled = int(round(y / 20))
                      # 3. 确保缩放后坐标非负（极端情况防护）
