# -----------------------------
# File: jetson_saliao_optimized.py
# Run on Jetson (Nano/Xavier)
# Features:
# - Camera capture + YOLOv8 instance segmentation
# - Coverage calculation (feed area)
# - Calculate next feed point
# - FFmpeg RTSP push
# - Robust serial thread: replies to 'ok' with X..Y.. or sends 'end' when coverage >= END_THRESHOLD
# - Clear logging and graceful shutdown
#
# Usage: python3 jetson_saliao_optimized.py
# Requires: ultralytics, opencv-python, numpy, pillow, pyserial
# -----------------------------
...
# def suggest_next_point(full_mask, feed_rect):
#     # returns (x,y) or None
#     x0, y0, x1, y1 = feed_rect
#     x0, y0 = max(0, x0), max(0, y0)
#     x1, y1 = min(full_mask.shape[1], x1), min(full_mask.shape[0], y1)
...
#     empty = (sub == 0).astype(np.uint8)
#     pts = cv2.findNonZero(empty)
#     if pts is None:
#         return None
#     avg = pts.mean(axis=0)[0]
#     return (int(avg[0]) + x0, int(avg[1]) + y0)
    ...
    try:
        while not stop_event.is_set():
            try:
                ...
                break
            if not raw:
                # periodically if coverage >= END_THRESHOLD, send end (even without ok)
                with pos_lock:
                    cov = latest_coverage
